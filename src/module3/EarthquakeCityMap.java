...
//			map = new UnfoldingMap(this, 200, 50, 700, 500, new Google.GoogleMapProvider());
	    ...
	    for (PointFeature featureItem : earthquakes) {
	    	Marker markerItem = this.createMarker(featureItem);
...
//		System.exit(0);
	    ...
	    if (mag < THRESHOLD_LIGHT) { // minor earthquakes
			// Here is an example of how to use Processing's color method to generate 
		    // an int that represents the color blue.  
		    int blue = color(0, 0, 255);
	    	marker.setColor(blue);
	    	marker.setRadius((float)5.0);
	    } else if ((THRESHOLD_LIGHT <= mag) && (mag < THRESHOLD_MODERATE)) { // light earthquakes
	    	int yellow = color(255,255,0);
	    	marker.setColor(yellow);
	    	marker.setRadius((float)10.0);
	    } else { // moderate earthquakes
