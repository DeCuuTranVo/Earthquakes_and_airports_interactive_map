...
//		earthquakesURL = "test2.atom";
	...
	private void addKey() {	
		...
		fill(color(136, 8, 8));
		int triangle_size = CityMarker.TRI_SIZE;
		triangle(50, 75-triangle_size, 50-triangle_size, 75+triangle_size, 50+triangle_size, 75+triangle_size);
		
		fill(color(255, 255, 255));
		ellipse(50, 90, triangle_size*2, triangle_size*2);
		rect(50-triangle_size, 105-triangle_size, triangle_size*2, triangle_size*2);
		fill(color(255, 255, 0));
		ellipse(50, 150, triangle_size*2, triangle_size*2);
		fill(color(0, 0, 255));
		ellipse(50, 165, triangle_size*2, triangle_size*2);
		fill(color(255, 0, 0));
		ellipse(50, 180, triangle_size*2, triangle_size*2);
		fill(color(255, 255, 255));
		ellipse(50, 195, triangle_size*2, triangle_size*2);
		line(50 - triangle_size, 195 - triangle_size, 50 + triangle_size, 195 + triangle_size);
		line(50 + triangle_size, 195 - triangle_size, 50 - triangle_size, 195 + triangle_size);
		
		
		fill(0, 0, 0);
		text("City Marker", 75, 75);
		text("Land Quake", 75, 90);
		text("Ocean Quake", 75, 105);
		text("Size ~ Magnitude", 50, 120);
		
		text("Shallow", 75, 150);
		text("Intermediate", 75, 165);
		text("Deep", 75, 180);
		text("Past day", 75, 195);
	...
	private void printQuakes() 
	{
		...
		for (Marker cm : countryMarkers) {
			...
			for (Marker qm :  quakeMarkers) {
				...
				if (em.isOnLand()) {
					String earthquakeCountry = (String) em.getProperty("country");
					if (earthquakeCountry.equals(countryName)){
						countryQuakeCounter++;
					}				
				}
			}
			
			if (countryQuakeCounter > 0) {
				System.out.println(countryName + ": " + countryQuakeCounter);
			}		
		}
